...
namespace modules {
  ...
  memory_module::memory_module(const bar_settings& bar, string name_) : timer_module<memory_module>(bar, move(name_)) {
    ...
    m_formatter->add(DEFAULT_FORMAT, TAG_LABEL, {TAG_LABEL, TAG_BAR_USED, TAG_BAR_RESERVED, TAG_BAR_FREE, TAG_BAR_AVAILABLE,
                                                 TAG_RAMP_USED, TAG_RAMP_RESERVED, TAG_RAMP_FREE, TAG_RAMP_AVAILABLE,
                                                 TAG_BAR_SWAP_USED, TAG_BAR_SWAP_FREE, TAG_RAMP_SWAP_USED, TAG_RAMP_SWAP_FREE});
    m_formatter->add_optional(FORMAT_WARN, {TAG_LABEL_WARN, TAG_BAR_USED, TAG_BAR_RESERVED, TAG_BAR_FREE, TAG_BAR_AVAILABLE,
                                                 TAG_RAMP_USED, TAG_RAMP_RESERVED, TAG_RAMP_FREE, TAG_RAMP_AVAILABLE,
                                                 TAG_BAR_SWAP_USED, TAG_BAR_SWAP_FREE, TAG_RAMP_SWAP_USED, TAG_RAMP_SWAP_FREE});
    ...
    if (m_formatter->has(TAG_BAR_RESERVED)) {
      m_bar_memreserved = load_progressbar(m_bar, m_conf, name(), TAG_BAR_RESERVED);
    }
    if (m_formatter->has(TAG_BAR_FREE)) {
      m_bar_memfree = load_progressbar(m_bar, m_conf, name(), TAG_BAR_FREE);
    }
    if (m_formatter->has(TAG_BAR_AVAILABLE)) {
      m_bar_memavail = load_progressbar(m_bar, m_conf, name(), TAG_BAR_AVAILABLE);
    }
    if(m_formatter->has(TAG_RAMP_USED)) {
      m_ramp_memused = load_ramp(m_conf, name(), TAG_RAMP_USED);
    }
    if(m_formatter->has(TAG_RAMP_RESERVED)) {
      m_ramp_memreserved = load_ramp(m_conf, name(), TAG_RAMP_RESERVED);
    }
    if(m_formatter->has(TAG_RAMP_FREE)) {
      m_ramp_memfree = load_ramp(m_conf, name(), TAG_RAMP_FREE);
    }
    if(m_formatter->has(TAG_RAMP_AVAILABLE)) {
      m_ramp_memavail = load_ramp(m_conf, name(), TAG_RAMP_AVAILABLE);
    }
  ...
  bool memory_module::update() {
    ...
    m_perc_memused = 100 - m_perc_memavail;
    m_perc_memreserved = 100 - m_perc_memfree;
    ...
    const auto replace_tokens = [&](label_t& label) {
      ...
      label->replace_token("%gb_reserved%", string_util::filesize_gib(kb_total - kb_free, 2, m_bar.locale));
      label->replace_token("%gb_free%", string_util::filesize_gib(kb_free, 2, m_bar.locale));
      label->replace_token("%gb_available%", string_util::filesize_gib(kb_avail, 2, m_bar.locale));
      ...
      label->replace_token("%mb_reserved%", string_util::filesize_mib(kb_total - kb_free, 0, m_bar.locale));
      label->replace_token("%mb_free%", string_util::filesize_mib(kb_free, 0, m_bar.locale));
      label->replace_token("%mb_available%", string_util::filesize_mib(kb_avail, 0, m_bar.locale));
      ...
      label->replace_token("%percentage_reserved%", to_string(m_perc_memreserved));
      label->replace_token("%percentage_free%", to_string(m_perc_memfree));
      label->replace_token("%percentage_available%", to_string(m_perc_memavail));
      ...
      label->replace_token("%reserved%", string_util::filesize_gib_mib(kb_total - kb_free, 0, 2, m_bar.locale));
      label->replace_token("%free%", string_util::filesize_gib_mib(kb_free, 0, 2, m_bar.locale));
      label->replace_token("%available%", string_util::filesize_gib_mib(kb_avail, 0, 2, m_bar.locale));
  ...
  bool memory_module::build(builder* builder, const string& tag) const {
    ...
    } else if (tag == TAG_BAR_RESERVED) {
      builder->node(m_bar_memreserved->output(m_perc_memreserved));
    } else if (tag == TAG_BAR_FREE) {
      builder->node(m_bar_memfree->output(m_perc_memfree));
    } else if (tag == TAG_BAR_AVAILABLE) {
      builder->node(m_bar_memavail->output(m_perc_memavail));
    ...
    } else if (tag == TAG_RAMP_AVAILABLE) {
      builder->node(m_ramp_memavail->get_by_percentage_with_borders(m_perc_memavail, 0, m_perc_memused_warn));
    } else if (tag == TAG_RAMP_USED) {
      builder->node(m_ramp_memused->get_by_percentage_with_borders(m_perc_memused, 0, m_perc_memused_warn));
    } else if (tag == TAG_RAMP_RESERVED) {
      builder->node(m_ramp_memreserved->get_by_percentage_with_borders(m_perc_memreserved, 0, m_perc_memused_warn));
